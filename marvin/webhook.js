...
const { currentStep } = require('./step.js');

const sorryMessage =
  "We're sorry but Liberandum doesn't recognize this number. For details visit http://liberandum.org/about.";

const addedUserMessage =
  'We are adding them to our list of potential recipients and will reach out to them as funds become available. Thanks so much for helping.';
...
module.exports = async (event: APIGatewayEvent, context: Context): Response => {
    ...
    switch (currentStep(userValues.value)) {
      ...
      case 4:
        ...
        g.V(user.value.id)
          .property(
            'has_place_to_stay',
            incoming.Body.trim().toLowerCase() === 'yes'
          )
          .next();
        break;
      case 5:
